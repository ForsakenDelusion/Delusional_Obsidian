...
async function readConfigFrontmatter() {
  ...
  try {
    ...
    if (!yamlMatch) {
      return { prefix: "Note_", noteStatus: "draft", categories: ["未分类"] };
    }
    ...
    const prefix = prefixMatch ? prefixMatch[1].trim() : "Note_";
    
    // 解析 defaults.note_status
    const statusMatch = yamlContent.match(/^defaults:\n[\s\S]*?^\s*note_status:\s*['"]?([^'"#\n]+?)['"]?\s*(?:#.*)?$/m);
    const noteStatus = statusMatch ? statusMatch[1].trim() : "draft";
    ...
    return { prefix, noteStatus, categories };
  } catch (err) {
...
if (title.startsWith("Untitled") || title.startsWith("未命名")) {
    const inputName = await tp.system.prompt("请输入卡片标题 (Note Name):");
