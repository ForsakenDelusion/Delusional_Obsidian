...
if (title.startsWith("Untitled") || title.startsWith("未命名")) {
    const inputName = await tp.system.prompt("请输入项目名称 (Project Name):");
    if (inputName) {
        title = inputName;
        await tp.file.rename(title);
    } else {
        title = "Project_" + tp.date.now("YYYYMMDD");
        await tp.file.rename(title);
    }
}

// --- 2. 截止日期 ---
const dueDateInput = await tp.system.prompt("截止日期? (格式 YYYY-MM-DD，留空回车跳过):");
const dueDate = dueDateInput ? dueDateInput : "";
-%>
---
type: project
status: active
...
> } else {
>     // 从配置读取角色标签映射
>     const roleLabels = configNote?.project_file_roles_labels || {
>         "log": "📋 Log",
>         "research": "🔍 Research", 
>         "resource": "📚 Resource"
>     };
