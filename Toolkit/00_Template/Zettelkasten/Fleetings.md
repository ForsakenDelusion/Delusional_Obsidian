...
async function readConfigFrontmatter() {
  ...
  try {
    ...
    if (!yamlMatch) {
      return { prefix: "Fleeting_", fleetingStatus: "fleeting", categories: ["未分类"] };
    }
    
    const yamlContent = yamlMatch[1];
    
    // 解析 prefixes.fleeting
    const prefixMatch = yamlContent.match(/^prefixes:\n[\s\S]*?^\s*fleeting:\s*['"]?([^'"#\n]+?)['"]?\s*(?:#.*)?$/m);
    const prefix = prefixMatch ? prefixMatch[1].trim() : "Fleeting_";
    
    // 解析 defaults.fleeting_status
    const statusMatch = yamlContent.match(/^defaults:\n[\s\S]*?^\s*fleeting_status:\s*['"]?([^'"#\n]+?)['"]?\s*(?:#.*)?$/m);
    const fleetingStatus = statusMatch ? statusMatch[1].trim() : "fleeting";
    ...
    return { prefix, fleetingStatus, categories };
  } catch (err) {
...
if (title.startsWith("Untitled") || title.startsWith("未命名")) {
    const inputName = await tp.system.prompt("请输入卡片标题 (Fleeting Name):");
